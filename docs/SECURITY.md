...
- **RSVP token**: an unguessable per-APPLICATION UUID that lets one accepted applicant confirm or decline attendance without logging in. It is NOT a column on `applications`: unlike the two tokens above the risk there is not anon PostgREST access but the `"Users read own applications"` SELECT policy, and RLS gates rows, not columns, so an `rsvp_token` column would be readable by the applicant's own session via `?select=rsvp_token`. It lives in the admin-only `application_rsvps` table with **no anon and no self-read policy at all** (migration 00065, modeled on 00054). The public page resolves via the service-role `getRsvpByToken()` action, which returns null uniformly on any miss (no existence oracle) and THROWS on a real DB error so an outage does not render every emailed link as a permanent 404. Defenses:
  ...
  - **No token leakage.** The page is `noindex, nofollow, nocache` and sets `Referrer-Policy: no-referrer`, so the token in the URL cannot escape via the Referer header.
