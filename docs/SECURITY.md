...
- **Walk-in registration token**: an unguessable per-chapter UUID that lets a person register a walk-in (auto-accepted) and create an account at the event. For the same reason as chapter communications (RLS gates rows, not columns, and `chapters` is publicly readable), the token is NOT a column on `chapters` — it would leak via `?select=walk_in_token`. It lives in a separate admin-only table `chapter_walk_in` with **no public/anon read policy** (migration 00054, modeled on 00052). The public walk-in page resolves the chapter from the token via the service-role `getWalkInChapterByToken()` action, which looks a chapter up *by* token and never exposes the token list; a miss returns null uniformly (no existence oracle). Defenses on `submitWalkInApplication()`:
  - **Token gate replaces the status gate.** The token (not `status === 'applications_open'`) authorizes the submission, so walk-ins work during hacking/submissions_open; only `draft`/`completed` chapters are refused for hygiene.
  - **Existing accounts need proof of ownership.** If the email already has a profile, the caller must prove they own it: either a session for that email, or the account's password in "I already have an account" mode, verified with `signInWithPassword` (which also signs them in). Otherwise the action refuses with `code: "account_exists"` and creates nothing. It never makes a second account and never touches the existing password. The password attempt is rate-limited per IP by `authLimiter` (scope `walk-in-login`), and admin/jury accounts are refused with the same generic message as `signIn()` before any password check. An existing application's check-in token is returned only to a proven owner. Revealing that an email has an account adds no oracle: the walk-in refusal already said so.
  - **Rate limiting.** Limited by IP (shared event WiFi uses the generous application limit) AND per walk-in token (`walkInTokenLimiter`, 60/min/token) so a leaked QR cannot be scripted into mass account creation.
  - **Rotation.** `rotateWalkInToken()` (chapter-admin guarded) issues a fresh UUID, invalidating any previously printed QR. Turnstile guards the public submission like other public forms.
- **Partner showcase token**: an unguessable per-chapter UUID that lets a sponsor view that match's consented applicants, CVs, teams/ranking, and photos without logging in. For the same reason as the walk-in token, it is NOT a column on `chapters` — it lives in the admin-only `chapter_partner_showcase` table with **no public/anon read policy** (migration 00060, modeled on 00054). The public page resolves the chapter via the service-role `getShowcaseByToken()` action, which returns null uniformly on any miss — no such token, disabled, or expired are indistinguishable (no existence oracle). Defenses:
  - **Consent is the hard gate.** Applicants appear only if they opted into sponsor sharing (`consent_sponsor_data` OR `consent_recruiting`). The predicate (`hasSponsorConsent` / `SPONSOR_CONSENT_OR_FILTER`) is applied both as a DB `.or()` filter and re-asserted in code (defence in depth), and is shared by the list and the CV proxy so a hidden applicant can never have their CV fetched.
  - **Explicit enablement + expiry.** `is_enabled` defaults false (an unshared showcase 404s), `show_cvs` defaults false (CV access is opt-in per chapter), and an optional `expires_at` hard-expires the link.
  - **CV proxy is chapter-scoped + keyed by application id.** `/api/showcase/<token>/cv/<applicationId>` resolves the chapter from the token, then `getShowcaseCvFileId()` returns the Drive file id only if the application belongs to THAT chapter and passes consent — so a token for chapter A cannot fetch a CV from chapter B (IDOR guard). Drive file ids never appear in URLs. CVs stream with `Cache-Control: no-store`, `X-Robots-Tag: noindex`, `Referrer-Policy: no-referrer`, `nosniff`.
  - **Rate limiting + no token leakage.** The CV route is IP-rate-limited (`showcaseLimiter`) to blunt scripted mass-download/enumeration. The page is `noindex` and sets `Referrer-Policy: no-referrer`; outbound profile/CV links carry `rel="noreferrer"` so the token in the URL cannot leak via the Referer header.
  - **Rotation.** `rotateShowcaseToken()` (chapter-admin guarded) issues a fresh UUID, immediately revoking the previously shared link.
  - **Bulk-CV ZIP** (`/api/showcase/<token>/cvs`): same gating chain as the single-CV proxy (live token via the rate-limited resolver, `show_cvs` on, consent-gated CV list with in-code re-check), PLUS a dedicated tight limiter (`showcaseZipLimiter`, 3/10min per IP) because one archive fans out to two Drive API calls per CV. Over-cap requests are refused with 413 before any bytes stream (never a silently truncated archive); mid-stream Drive failures are skipped and listed in a `_MANIFEST.txt`. Same no-store/noindex/no-referrer hygiene.
- **RSVP token**: an unguessable per-APPLICATION UUID that lets one accepted applicant confirm or decline attendance without logging in. It is NOT a column on `applications`: unlike the two tokens above the risk there is not anon PostgREST access but the `"Users read own applications"` SELECT policy, and RLS gates rows, not columns, so an `rsvp_token` column would be readable by the applicant's own session via `?select=rsvp_token`. It lives in the admin-only `application_rsvps` table with **no anon and no self-read policy at all** (migration 00065, modeled on 00054). The public page resolves via the service-role `getRsvpByToken()` action, which returns null uniformly on any miss (no existence oracle) and THROWS on a real DB error so an outage does not render every emailed link as a permanent 404. Defenses:
  - **The page never writes. This is the load-bearing property.** Mail scanners (Outlook Safe Links, provider link-prefetchers) fetch every URL in an email before the recipient sees it, so an answer recorded on GET would be supplied by a robot for every scanned mailbox. `getRsvpByToken()` is strictly read-only and the answer is written exclusively by the `submitRsvp()` POST action behind a deliberate click. `tests/rsvp-no-write-on-get.test.ts` pins this by asserting zero mutating calls on the resolver. The same class of bug already bit password reset (see `lib/auth-errors.ts`, which names it in user-facing copy).
  - **A 48h window, enforced by the database.** `submitRsvp()` adds `email_sent_at > now() - RSVP_WINDOW_HOURS` to the same UPDATE that records the answer, so the window cannot be beaten by a race or by a stale page left open. Expiry is DERIVED from `email_sent_at`, so there is no stored copy to drift. Unlike the showcase token, an expired RSVP does NOT collapse into a uniform 404: this token guards only a one-bit self-report by the single person it was mailed to, so the (minimal) disclosure that their own link was once valid is worth far less than telling them why they cannot answer. An unknown token stays a uniform failure.
  - **Write-once, enforced by the database.** `submitRsvp()` updates `WHERE rsvp_token = ? AND response IS NULL`, so the first answer wins even under a concurrent double submit; there is no read-then-write gap to race. A submit against an answered token reports the standing answer rather than overwriting it.
  - **Strict value validation.** Only the literals `"yes"` and `"no"` are accepted, checked in code before any write and again by a `CHECK` constraint on the column.
  - **Dual rate limiting.** Per IP (`rsvpLimiter`) on both the resolver and the write, and per token (`rsvpTokenLimiter`) on the write, so a leaked or forwarded link cannot be scripted against the write path. The resolver rate-limits BEFORE any DB work, so it cannot serve as an unthrottled token-validity oracle.
  - **No token leakage.** The page is `noindex, nofollow, nocache` and sets `Referrer-Policy: no-referrer`, so the token in the URL cannot escape via the Referer header. `rsvp` is also in `SECRET_PATH_SEGMENTS` (`lib/utils.ts`), so a client-error report or stack trace carrying the page URL is redacted to `/rsvp/<redacted>` before it reaches the append-only `event_log`.
  - **Malformed tokens are rejected on shape, before the query.** Postgres rejects a non-uuid literal in `rsvp_token = ?` rather than matching no rows, and this module throws on DB errors, so an unvalidated token rendered a 500 instead of a 404 (mail clients wrap and truncate long URLs, so real recipients hit this). Both entry points shape-check first, which also keeps every miss UNIFORM: a garbage token, a truncated one and an unknown one are indistinguishable.
  - **Blast radius is nil by design.** The token grants exactly one capability: setting a one-bit attendance flag that no other system reads. It cannot change status, check anyone in, read personal data beyond the recipient's own first name and the public chapter details, or affect teams or registration.
  - **Admin side is chapter-scoped.** `sendRsvpEmails()` and `getRsvpsForChapter()` both call `requireChapterAdminAction(chapterId)` before any DB work, so a local chapter admin cannot mail or read another chapter (`tests/rsvp-idor.test.ts`).
- **Grand Finale invite token**: an unguessable per-PERSON UUID that lets one member of a qualifying team accept their Finale place without logging in. Same table reasoning as the RSVP token (RLS gates rows, not columns) plus one more: the invite exists BEFORE any application does, so it cannot be a column on a row that is not there yet. It lives in the admin-only `finale_invites` table with **no anon and no self-read policy** (migration 00068). The stakes are higher than for the RSVP: a write here ACCEPTS the person and mails a check-in QR. Defenses:
  - **The page never writes.** `getFinaleInviteByToken()` is strictly read-only and the answer is written exclusively by the `respondToFinaleInvite()` POST action behind a deliberate click, so a mail scanner's prefetch cannot register finalists who never clicked (`tests/finale-invite-no-write-on-get.test.ts`).
  - **Write-once, enforced by the database.** The update carries `WHERE invite_token = ? AND response IS NULL`, so a double click or a race produces one answer and therefore at most one application. `UNIQUE (chapter_id, user_id)` means a person on two qualifying teams cannot hold two invites, and `UNIQUE (chapter_id, email)` on `applications` means "yes" can never insert a duplicate application.
  - **A public link never over-promotes.** A `cancelled` application is left untouched (an organiser removed that person deliberately); only `pending`, `waitlisted` and `rejected` are promoted to accepted.
  - **Consent is explicit.** The page states above the buttons that "I'm in" registers them, confirms attendance and accepts the privacy policy; the optional consents (newsletter, recruiting, media) are copied from their earlier application or left false, never invented.
  - **Dual rate limiting**, same limiters as the RSVP: per IP on the resolver and the write, per token on the write.
  - **No token leakage.** The page is `noindex, nofollow, nocache` with `Referrer-Policy: no-referrer`, and `finale` is in `SECRET_PATH_SEGMENTS` (`lib/utils.ts`), so a client-error report carrying the URL is redacted to `/finale/<redacted>` before it reaches the `event_log`.
  - **Malformed tokens are rejected on shape, before the query**, for the same reason as the RSVP token.
  - **Sending is GLOBAL admin only.** `sendFinaleInvites()` and `getFinaleInviteBoard()` call `requireAdminAction()`, because the qualifying teams come from the season leaderboard and reach across every chapter, not just one chapter's applicants. The action also refuses any chapter that is not `is_finale`.
- **Loyalty bonus**: `loyalty_bonuses` (migration 00067) holds hand-awarded season bonus points per team. RLS is on with **no policies at all**, so anon and authenticated sessions read zero rows and cannot write; only the service role manages it. The public sees a bonus only through the `leaderboard` view, which runs with its owner's rights (like the view did before) and exposes just the `loyalty_bonus` amount, never the `reason` or `created_at`. Supabase's `security_definer_view` lint flags that view; this is intentional, because switching it to invoker rights would hide published scores and bonuses from public readers.
...
- Certificate routes accept EITHER session auth (team member or admin) OR an
  unguessable capability token bound to the exact chapter, team, member, and
  certificate variant (see Cryptography below); they only ever serve published
  scores. The token path lets emailed certificate links open without login while
  preventing access to another member's certificate.
...
- **Identity is an explicit username, not a guess.** The collaborator is resolved from `profiles.github_username`, captured (optionally) at invite time. The prior behavior resolved jurors by searching GitHub for their account email, which matched only users with a public profile email and returned the *first* search hit: a wrong or absent match either granted nobody access or, in principle, granted the wrong account access to participant code. That email search remains only as a fallback for jurors with no stored username.
- **A missing username fails closed.** It is never a blocker on inviting a juror; it simply means no collaborator invite is created, so the juror cannot open the private fork. Jurors missing one are flagged in the admin jury list for challenges that judge private repos (`lib/jury-github.ts: juryGitHubUsernameNeeded`). No access is ever widened to compensate: the fork is not made public and no shared credential is handed out.
...
- **Algorithm**: HMAC-SHA256 over the chapter, team, certificate variant, and
  member scope, base64url-encoded
...
- **Scope**: a current token authorizes ONLY one member's exact certificate
  variant. Changing the chapter, team, member, or variant invalidates it. Legacy
  team-scoped tokens remain accepted only by the old default URL so links from
  emails already sent do not break.
- **No DB/migration**: stateless. Rotating the secret invalidates all previously emailed links at once; logged-in admins/members are unaffected (their session path is unchanged). There is no token expiry by design; the link's only sensitive content is the member's own already-published certificate.
