...
import { signInAdminWithGoogle, signInChapterAdminWithGoogle } from "@/lib/actions/auth";
...
function LoginForm() {
  ...
  async function handleChapterAdminSignIn() {
    setError(null);
    setLoading("chapter");
    const result = await signInChapterAdminWithGoogle();
    if (result?.error) {
      setError(result.error);
      setLoading(null);
    }
  ...
  return (
    <Card className="w-full max-w-sm">
      ...
      <div className="mt-6 flex flex-col gap-3">
        ...
        >
          <GoogleIcon />
          {loading === "admin" ? "Redirecting..." : "Sign in with Google"}
        </button>

        <div className="relative flex items-center gap-3">
          <div className="h-px flex-1 bg-white/10" />
          <span className="text-xs text-text-muted">chapter admin?</span>
          <div className="h-px flex-1 bg-white/10" />
        </div>

        <button
          onClick={handleChapterAdminSignIn}
          disabled={loading !== null}
          className="flex w-full items-center justify-center gap-3 rounded-lg border border-white/10 bg-surface-deep px-4 py-3 font-medium text-text-primary transition-all hover:bg-white/5 hover:border-white/20 disabled:opacity-50"
        >
          <GoogleIcon />
          {loading === "chapter" ? "Redirecting..." : "Sign in as Chapter Admin"}
        </button>
